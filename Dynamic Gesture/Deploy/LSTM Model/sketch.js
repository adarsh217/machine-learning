...
async function preload(){
  brain = await tf.loadLayersModel('LSTM Model/model.json');
...
async function setup() {
  ...
  while(1){ 
    await new Promise(next => {
      ...
      fill_inputs(); // add the current poses to the inputs

      async function start(){
        if(inputs.length == TIME_STEP && fill_inputs_done){
          ...
          r.data().then(function(label){ 
            if(label[0] > 0.85){
            res = "jumping\n";
            }else if(label[1] > 0.85){
              res = "left waving\n";
            }else if(label[3] > 0.85){
              res = "right waving\n";
            }else{
              res = "neutral";
            }
